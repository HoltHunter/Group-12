...
function Header(session, signOut) {
    ...
    const respondRequest = (userId, friendRequestId, decision, e) => {
        ...
        const reqBody = JSON.stringify({ request_id: friendRequestId, decision })
        ...
        <div className="ui secondary pointing menu">
            ...
            { session && (
                <div className="ui icon top left simple dropdown button">
                    <i className={filterFriends().length > 0 ? "bell green icon" : "bell icon"} />
                    <div className="menu">
                            ...
                            filterFriends().map((user) => (
                                <div key={user.id} className="item">
                                    ...
                                    <div className="ui small basic icon buttons">
                                        <button className="ui button" onClick={(e) => respondRequest(user.id, user.friend_request_id, true, e)}><i className="checkmark green icon" /></button>
                                        <button className="ui button" onClick={(e) => respondRequest(user.id, user.friend_request_id, false, e)}><i className="x red icon" /></button>
                                    </div>
                                </div>
                            ))
                        }
